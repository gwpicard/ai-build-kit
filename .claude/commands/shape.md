...
When this command comes from a Claude plugin, load and follow `${CLAUDE_PLUGIN_ROOT}/.agents/skills/shape/SKILL.md`. Otherwise, load and follow `.agents/skills/shape/SKILL.md`. It is the single source of truth for the `/shape` command. Treat anything typed after the command as the user's request and pass it through unchanged.
