...
to run it, which the kit knows well enough to check at launch, and
`.agents/skills/ship/references/recipe-format.md` says what one holds. The menu
is the files directly in `.agents/skills/ship/recipes/`, read now rather than
remembered. Nothing else is on it: not the `parts/` folder, and not a recipe
kept anywhere else while it waits for its real run. Read each file's `Fits:`
line and keep the ones that fit the shape.

Where recipes fit, show them with exactly one recommended, chosen by its
`Recommended when:` line against what the interview said. For each, say in
plain words what it promises: the launch steps the kit can check on it, such as
preview, rollback, backup and restore. Say what it costs to run in the same
plain words, meaning which accounts the person will hold and whether the tool
runs on a hosting platform or on a server they rent. Never quote a price. Take
every product name from the recipe file at this moment, and never write one
into this skill. Say that they may bring their own stack instead.
...
none`, and do not raise it again. If they give no answer, or say to get on with
it, take the recommended recipe, say so in one line, and carry on. The menu is
never a condition of founding. Where no recipe fits the shape, such as a
command-line tool or a desktop app, say so in one line, record `Recipe: none`,
and set up as below without a menu.

Once a recipe is chosen, build on its `Build stack:` line and record `Recipe:
<file name>` in AGENTS.md's stack section. Run `scripts/check-tooling.sh
--recipe <recipe file>` from this installed skill folder. A tool it reports
missing is needed before the first /ship, not now: name it once, add it to the
masterplan as a setup task, and carry on. That report never stops founding.

Without a recipe, set up accordingly: one established, conventional stack,
because the agent is strongest where the conventions run deepest. On any stack,
use managed services for anything storing sign-ins, payments, or files;
...
uses; on a recipe, its preview section says how. Where a hosting companion or whoever runs the server will host it, /ship
