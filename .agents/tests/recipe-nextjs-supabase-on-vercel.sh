...
rs_rule "the kit reads a protected preview's health" 'the kit reads the preview.s health with .vercel curl /api/health --deployment <preview address> --yes., which gets past that sign-in'
rs_rule "the command is named as beta" 'that command is marked beta'
rs_rule "the bypass token is said before it is made" 'it creates a deployment protection bypass token on the project and prints no token, and the kit says so before running it'
rs_rule "without it the person opens the preview signed in" 'when the command is missing or fails, the person opens the address in their own browser, signed in to vercel'
...
rs_rule ".env.example is checked as still tracked" '.git ls-files \.env\.example. names the file'
