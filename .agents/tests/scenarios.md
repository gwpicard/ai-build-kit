...
- Escalation: asking about the menu a second time, ending a turn to wait for a menu answer, or holding the stand-up until one arrives fails the case. So do leaving the menu out of the conversation, recommending both recipes or neither, quoting a price, recording `Recipe: none` or a recipe other than the recommended one, and a `founding-menu` line that leaves out a file on the menu. What the monthly visit later does with that line is outside this case.
